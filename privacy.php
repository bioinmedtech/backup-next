...
<main class="mx-auto max-w-6xl px-6 py-10 md:px-10 md:py-14">
    <section class="pb-8">
        <div class="max-w-4xl">
            <p class="text-[0.72rem] font-semibold uppercase tracking-[0.18em] text-[#2a5a94]"><?php echo e($privacyMeta['eyebrow'] ?? 'Правовая информация'); ?></p>
            <h1 class="mt-2 text-[1.8rem] font-bold leading-tight text-[#0f2749] md:text-[2.6rem]"><?php echo e($privacyMeta['title'] ?? 'Политика конфиденциальности'); ?></h1>
            ...
            <div class="mt-4 flex flex-col gap-1 text-[0.82rem] text-[#0a293c] sm:flex-row sm:flex-wrap sm:gap-4">
                <p><span class="font-semibold text-[#2a5a94]"><?php echo e($privacyMeta['effective_date_label'] ?? 'Дата вступления в силу:'); ?></span> <?php echo e($privacyMeta['effective_date'] ?? '30.04.2026'); ?></p>
                <p><span class="font-semibold text-[#2a5a94]"><?php echo e($privacyMeta['updated_date_label'] ?? 'Дата последнего изменения:'); ?></span> <?php echo e($privacyMeta['updated_date'] ?? '30.04.2026'); ?></p>
            </div>
        </div>

        <div class="mt-8 grid gap-5 border-y border-[#d8e6f3] py-6 md:grid-cols-3">
            <div>
                <p class="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-[#2a5a94]"><?php echo e($privacyCards['operator']['title'] ?? 'Оператор данных'); ?></p>
                <p class="mt-2 text-[0.95rem] leading-relaxed text-[#355b89]"><?php echo e(CLINIC_NAME); ?><br><?php echo e(CLINIC_ADDRESS); ?></p>
            </div>
            <div>
                <p class="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-[#2a5a94]"><?php echo e($privacyCards['contacts']['title'] ?? 'Контакты'); ?></p>
                <p class="mt-2 text-[0.95rem] leading-relaxed text-[#355b89]"><?php echo e(CLINIC_PHONE); ?><br><?php echo e(CLINIC_EMAIL); ?></p>
            </div>
            <div>
                <p class="text-[0.72rem] font-semibold uppercase tracking-[0.14em] text-[#2a5a94]"><?php echo e($privacyCards['purpose']['title'] ?? 'Назначение обработки'); ?></p>
                <p class="mt-2 text-[0.95rem] leading-relaxed text-[#355b89]"><?php echo e($privacyCards['purpose']['text'] ?? ''); ?></p>
            </div>
        </div>
            ...
            <?php foreach ($privacySections as $index => $section): ?>
                ...
                <section class="<?php echo $isLast ? '' : 'border-b border-[#e2ecf5] pb-6'; ?>">
                    ...
                    <?php foreach ($paragraphs as $paragraph): ?>
                        <?php
                        $line = str_replace('{{clinic_email}}', CLINIC_EMAIL, (string)$paragraph);
                        ?>
